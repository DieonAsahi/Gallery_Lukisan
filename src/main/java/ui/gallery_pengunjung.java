 ...
 * @author athaw
 */
public class gallery_pengunjung extends javax.swing.JFrame implements LanguageUpdateListener {
    ...
    public gallery_pengunjung(object.User up) {
        initComponents();
        setupUI();
        setLocationRelativeTo(null);
        loadData();
    ...
    // Proses kunjungan jika role user adalah pengunjung
    private void prosesKunjungan() {
        User user = SessionManager.get();

        if (user == null) {
            System.out.println("[DEBUG] User dari session NULL");
            return;
        }

        System.out.println("[DEBUG] User login: " + user.getUsername());
        System.out.println("[DEBUG] Role: " + user.getRole());

        if ("pengunjung".equalsIgnoreCase(user.getRole())) {
            int userId = getUserId(user.getUsername());
            System.out.println("[DEBUG] ID dari DB: " + userId);
            if (userId != -1) {
                simpanKunjungan(userId);
            }
        } else {
            System.out.println("[DEBUG] Role bukan pengunjung, tidak simpan kunjungan.");
        }
    }

    // Ambil ID user dari database berdasarkan username
    ...
    private void simpanKunjungan(int userId) {
        try (Connection conn = config.configDB()) {
            String insert = "INSERT INTO kunjungan (user_id) VALUES (?)";
            PreparedStatement pst = conn.prepareStatement(insert);
            pst.setInt(1, userId);
            pst.executeUpdate();
            System.out.println("[DEBUG] Kunjungan berhasil disimpan untuk user_id: " + userId);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, "Gagal menyimpan kunjungan: " + e.getMessage());
        }
    }
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addComponent(jPanel7, javax.swing.GroupLayout.DEFAULT_SIZE, 1022, Short.MAX_VALUE))
                .addContainerGap())
        );
